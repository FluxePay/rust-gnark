//! Rust bindings for gnark Groth16 and PLONK BN254 prover/verifier.
//!
...
/// Result of a PLONK BN254 proof generation.
///
/// Both fields are hex-encoded binary serializations from gnark:
/// - `proof`: the PLONK proof (via `proof.WriteTo()`)
/// - `public_inputs`: the public witness (via `witness.MarshalBinary()`)
#[derive(Debug, Clone)]
...
/// Generate a PLONK BN254 proof.
///
/// # Arguments
///
/// * `scs_path` - Path to the `.scs` file (sparse constraint system, gnark binary format)
...
/// # Returns
///
/// A [`PlonkProofResult`] containing hex-encoded proof and public inputs.
///
/// # Errors
///
/// Returns an error if file loading, witness construction, or proof generation fails.
pub fn plonk_prove(
    ...
    pk_path: &str,
    witness_json: &str,
) -> Result<PlonkProofResult> {
    let scs = CString::new(scs_path)?;
    let pk = CString::new(pk_path)?;
    let witness = CString::new(witness_json)?;

    let result = unsafe {
        bind::gnark_plonk_prove(
            scs.as_ptr() as *mut c_char,
            pk.as_ptr() as *mut c_char,
            witness.as_ptr() as *mut c_char,
        )
    };
    ...
    if result.is_null() {
        bail!("gnark_plonk_prove returned null");
    }
    ...
    unsafe {
        let out = if (*result).error.is_null() {
            ...
            if proof_ptr.is_null() || pub_ptr.is_null() {
                Err(anyhow::anyhow!(
                    "gnark_plonk_prove: proof or public_inputs pointer is null"
                ))
            } else {
                Ok(PlonkProofResult {
                    proof: CStr::from_ptr(proof_ptr).to_string_lossy().into_owned(),
            ...
            let err = CStr::from_ptr((*result).error)
                .to_string_lossy()
                .into_owned();
            Err(anyhow::anyhow!("PLONK proof generation failed: {err}"))
        };

        bind::gnark_free_plonk_proof_result(result);
        out
    }
}

/// Verify a PLONK BN254 proof.
///
/// # Arguments
///
/// * `scs_path` - Path to the `.scs` file
/// * `vk_path` - Path to the `.vk` file (PLONK verifying key)
/// * `proof_result` - The proof result from [`plonk_prove`]
///
/// # Returns
///
/// `Ok(true)` if the proof is valid, `Ok(false)` if the proof is invalid,
